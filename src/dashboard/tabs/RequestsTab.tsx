        ...
        <table className="requests-table">
          ...
          <tbody>
            {rows.map((r, i) => {
              ...
              return (
                <tr key={i}>
                  <td style={{ fontSize: '0.75rem' }}>{r.protocol}</td>
                  <td className="endpoint">{r.endpoint}</td>
                  ...
                  <td style={{ fontSize: '0.75rem', color: 'var(--text-secondary)' }}>{r.last_responded ? new Date(r.last_responded).toLocaleString() : 'N/A'}</td>
                </tr>
              );
            })}
          </tbody>
